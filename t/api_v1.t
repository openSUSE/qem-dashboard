...
subtest 'No incidents yet' => sub {
  stderr_like {
    $t->get_ok('/api/v1/incidents'       => $auth_headers)->status_is(200)->json_is('',       [], 'empty list');
    $t->get_ok('/api/v1/incidents/1'     => $auth_headers)->status_is(404)->json_is('/error', 'Incident not found');
    $t->get_ok('/api/v1/incidents/16860' => $auth_headers)->status_is(404)->json_is('/error', 'Incident not found');
  }
  qr/access_log/, 'access log caught';
};
...
subtest 'Compression available' => sub {
  ...
  stderr_like {
    $t->get_ok('/api/v1/incidents' => $auth_headers)
      ->status_is(200)
...
subtest 'Add and Update incidents' => sub {
  stderr_like {
    $t->patch_ok('/api/v1/incidents' => $auth_headers => json => [$mock_incident])
      ->status_is(200)
      ->json_is('/message', 'Ok');

    my $expected = {%$mock_incident, type => '', url => '', scminfo => ''};
    $t->get_ok('/api/v1/incidents'       => $auth_headers)->status_is(200)->json_is('', [$expected]);
    $t->get_ok('/api/v1/incidents/16860' => $auth_headers)->status_is(200)->json_is('', $expected);

    # Update
    my $updated_mock = {%$mock_incident, priority => 456, embargoed => true};
    $t->patch_ok('/api/v1/incidents/16860' => $auth_headers => json => $updated_mock);
    diag $t->tx->res->body unless $t->tx->res->code == 200;
    $t->status_is(200)->json_is('/message', 'Ok');

    $t->get_ok('/api/v1/incidents/16860' => $auth_headers)
      ->status_is(200)
    ...
    $t->patch_ok('/api/v1/incidents' => $auth_headers => json => [$qem_bot_incident])
      ->status_is(200)
      ->json_is('/message', 'Ok');
  }
  qr/access_log/, 'access log caught';
...
subtest 'Settings and Jobs' => sub {
  stderr_like {
    ...
    $t->put_ok(
      '/api/v1/incident_settings' => $auth_headers => json => {
        incident      => 16860,
        version       => '12-SP5',
    ...
    $t->get_ok('/api/v1/incident_settings/99999' => $auth_headers)
      ->status_is(400)
      ->json_is('/error', 'Incident not found');

    # Add update settings
    $t->put_ok(
      '/api/v1/update_settings' => $auth_headers => json => {
        incidents => [16860],
        product   => 'SLES-15-GA',
        arch      => 'x86_64',
        build     => '20201107-1',
    ...
    $t->put_ok('/api/v1/jobs' => $auth_headers => json => $job_data)->status_is(200)->json_is('/message', 'Ok');
    ...
    $t->put_ok('/api/v1/jobs' => $auth_headers => json => $job_data_2)->status_is(200)->json_is('/message', 'Ok');

    # Verify Get jobs by settings
    $t->get_ok('/api/v1/jobs/incident/1' => $auth_headers)->status_is(200);
    ...
    $t->put_ok('/api/v1/jobs' => $auth_headers => json => $job_data_same_group)->status_is(200);
...
subtest 'Remarks' => sub {
  stderr_like {
    $t->patch_ok('/api/v1/jobs/4953193/remarks?incident_number=16860&text=acceptable_for' => $auth_headers)
      ->status_is(200)
      ->json_is('/message', 'Ok');

    $t->get_ok('/api/v1/jobs/4953193/remarks' => $auth_headers)
      ->status_is(200)
      ->json_is('/remarks/0/text', 'acceptable_for');

    # Branch coverage: job remark without incident number
    $t->patch_ok('/api/v1/jobs/4953193/remarks?text=global_remark' => $auth_headers)
      ->status_is(200)
      ->json_is('/message', 'Ok');
    $t->get_ok('/api/v1/jobs/4953193/remarks' => $auth_headers)
      ->status_is(200)
    ...
    $t->get_ok('/api/v1/jobs/8888888/remarks' => $auth_headers)
      ->status_is(404)
      ->json_is('/error', 'openQA job (8888888) does not exist');

    # Missing branch: non-existent incident
    $t->patch_ok('/api/v1/jobs/4953193/remarks?incident_number=99999&text=foo' => $auth_headers)
      ->status_is(404)
...
subtest 'Extra Settings Coverage' => sub {
  stderr_like {
    ...
    $t->put_ok('/api/v1/incident_settings' => $auth_headers => json =>
      ...
      ->status_is(200);
    $t->get_ok('/api/v1/incident_settings/16860' => $auth_headers)
      ->status_is(200)
...
subtest 'Authentication' => sub {
  stderr_like {
    $t->get_ok('/api/v1/incidents')->status_is(200);
    $t->patch_ok('/api/v1/incidents' => json => [])->status_is(403)->json_is('/error', 'Permission denied');
  }
  qr/access_log/, 'access log caught';
};
